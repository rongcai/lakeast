...
本框架对粒子群算法与遗传算法进行逻辑解耦，对其中的改进点予以封装，进行模块化，使用者可以采取自己对该模块的改进替换默认实现组成新的改进算法与已有算法进行对比试验。试验结果基于Excel文件输出，并可通过设定不同的迭代结束方式选择输出迭代次数变化的平均达优率结果（设定终止条件区间大于0）还是输出随迭代次数变化的平均最优值结果（设定终止条件区间等于0）。
...
9 各个测试算法都被加入到TestBatch以后，batch.run()开始执行算法比较过程并输出结果Excel文件到C盘根目录（输出路径可在Testable接口中配置，除了生成Excel文件外，还可以通过修改log4j.properties修改产生日志）。